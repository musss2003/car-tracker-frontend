...
const CarsPage = () => {
  ...
  const fetchCars = async () => {
    try {
      setLoading(true)
      const [carsData, activeContracts] = await Promise.all([getCars(), getActiveContracts()])

      // Store busy cars
      const busyPlates = new Set(activeContracts.map((contract) => contract.car?.licensePlate).filter((plate) => plate))
  ...
  const filteredAndSortedCars = useMemo(() => {
    ...
    if (sortConfig.key) {
      result.sort((a, b) => {
        ...
        aValue = typeof aVal === "string" || typeof aVal === "number" || aVal instanceof Date ? aVal : ""
        bValue = typeof bVal === "string" || typeof bVal === "number" || bVal instanceof Date ? bVal : ""
  ...
  const renderStatusBadge = (isBusy: boolean) => {
    return (
      <Badge variant={isBusy ? "destructive" : "default"} className="flex items-center gap-1">
        {isBusy ? (
          <>
      ...
      <div className="border-b bg-card">
        <div className="px-6 py-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <h1 className="text-3xl font-bold tracking-tight">Upravljanje vozilima</h1>
            <div className="flex gap-2">
              <DropdownMenu>
                ...
                <DropdownMenuContent className="bg-background-100/80 dark:bg-background-100/80 backdrop-blur-sm rounded-md shadow-md">
                  <DropdownMenuItem onClick={exportToPDF}>Izvezi kao PDF</DropdownMenuItem>
                  <DropdownMenuItem onClick={exportToExcel}>Izvezi kao Excel</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
          ...
          <div className="space-y-4">
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="hidden min-[800px]:table-cell">Status</TableHead>
                      ...
                      <TableRow key={car.id || index}>
                        <TableCell>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium">{car.manufacturer || "N/A"}</span>
                          </div>
                        </TableCell>
                        ...
                        <TableCell className="hidden min-[800px]:table-cell">{renderStatusBadge(car.isBusy)}</TableCell>
                        <TableCell>
                          <div className="flex gap-2">
                            <Button
                              variant="ghost"
                            ...
                            <Button
                              ...
                              title="Pogledaj dostupnost"
                            >
                              <CalendarIcon className="w-4 h-4" />
                            </Button>
                            <Button variant="ghost" size="icon" onClick={() => handleEdit(car)} title="Uredi vozilo">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="text-sm text-muted-foreground">
                Prikazujem {paginatedCars.length} od {filteredAndSortedCars.length} vozila
              </div>
              <div className="flex items-center gap-2">
                <Select
            ...
            <AlertDialogDescription>
              ...
              {carToDelete && (
                <span className="font-medium">
                  {" "}
                  {carToDelete.manufacturer} {carToDelete.model} ({carToDelete.licensePlate})
                </span>
              )}
              .
            </AlertDialogDescription>
