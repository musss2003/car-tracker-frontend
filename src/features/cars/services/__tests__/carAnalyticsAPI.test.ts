...
describe('Car Analytics API Service', () => {
  ...
  describe('Cost Analytics', () => {
    it('should fetch cost analytics for a car', async () => {
      const mockResponse = {
        totalCosts: { all: 2000, service: 1000, registration: 200, insurance: 500, issues: 300 },
        monthlyCosts: [{ month: '2025-01', year: 2025, service: 100, registration: 0, insurance: 0, issues: 50, total: 150 }],
        categoryBreakdown: [{ category: 'Service', amount: 1000, percentage: 50 }],
      ...
      expect(inputValidator.validateId).toHaveBeenCalledWith('car-123', 'car id');
      expect(api.get).toHaveBeenCalledWith('/api/cars/car-123/cost-analytics', 'cost analytics', 'car-123');
  ...
  describe('Maintenance Alerts', () => {
    it('should fetch maintenance alerts', async () => {
      ...
      expect(api.get).toHaveBeenCalledWith('/api/cars/car-456/maintenance-alerts', 'maintenance alerts', 'car-456');
  ...
  describe('Dashboard', () => {
    it('should fetch dashboard data', async () => {
      const mockResponse = {
        ...
        maintenanceAlerts: { alerts: [], summary: { critical: 0, warnings: 0, total: 0 } },
        recentActivity: { latestService: null, latestRegistration: null, latestInsurance: null, recentIssues: [] },
      };
      ...
      expect(api.get).toHaveBeenCalledWith('/api/cars/car-789/dashboard', 'car dashboard', 'car-789');
  ...
  describe('Top Expenses', () => {
    ...
    it('should validate limit range', async () => {
      await expect(getTopExpenses(0)).rejects.toThrow('Limit must be between 1 and 100');
      await expect(getTopExpenses(101)).rejects.toThrow('Limit must be between 1 and 100');
    });
  });
  ...
  describe('Maintenance Summary', () => {
    it('should fetch fleet summary', async () => {
      ...
      expect(api.get).toHaveBeenCalledWith('/api/cars/maintenance/summary', 'maintenance summary');
